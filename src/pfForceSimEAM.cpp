...
double pfHome::forceEAM(vector<Func>& ffs, int tag) {
  while (true) {
    ...
    for (int i = 0; i < nfuncs; i++) {
      MPI_Bcast(&ffs[i].yy.front(), ffs[i].yy.size(), MPI_DOUBLE, PFROOT,
                MPI_COMM_WORLD);
      MPI_Bcast(&ffs[i].g1.front(), ffs[i].g1.size(), MPI_DOUBLE, PFROOT,
                MPI_COMM_WORLD);
    }
    ...
    for (int cc = locstt; cc < locend; cc++) {
      ...
      for (int ii = 0; ii < cnf.natoms; ii++) {
        ...
        for (int nn = 0; nn < atm.nneighs; nn++) {
          ...
          if (ngb.r < ffs[PHI].xx.back()) {
            ...
            splint(ffs[PHI], ngb.slots[0], ngb.shifts[0], ngb.steps[0], phi,
                   phigrad);
          ...
          if (ngb.r < ffs[RHO].xx.back()) {
            double rho;
            splint(ffs[RHO], ngb.slots[1], ngb.shifts[1], ngb.steps[1], rho,
                   ngb.rhog);
            atm.rho += rho;
        ...
        if (atm.rho < ffs[EMF].xx.front()) {
          ...
          splint(ffs[EMF],
                 ffs[EMF].xx[0] - 0.5 * (ffs[EMF].xx[1] - ffs[EMF].xx[0]), embE,
                 atm.gradF);
          embE += atm.gradF * extra;
        } else
      ...
      err += square11(fitengy - cnf.engy);
    }  // cc

    MPI_Reduce(&err, &gerr, 1, MPI_DOUBLE, MPI_SUM, PFROOT, MPI_COMM_WORLD);
    MPI_Reduce(&minrho, &gminrho, 1, MPI_DOUBLE, MPI_MIN, PFROOT,
               MPI_COMM_WORLD);
    MPI_Reduce(&maxrho, &gmaxrho, 1, MPI_DOUBLE, MPI_MAX, PFROOT,
               MPI_COMM_WORLD);
    if (mrank == PFROOT) {
